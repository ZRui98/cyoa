...
export async function getUserFromDb(
    ...
    Object.entries(values).forEach(([key, val]) => {
      transaction = transaction.where(key as ReferenceExpression<Database, 'user'>, '=', val)
    })
    return await transaction.selectAll()
      .executeTakeFirst()
};

export async function insertUserDb(user: Insertable<UserData>,
    trx: Transaction<Database> | Kysely<Database> = db,): Promise<Selectable<UserTable>> {
...
export async function activateUser(
    ...
    const result = await updateUserDb({...options, activated: true}, name, trx);
