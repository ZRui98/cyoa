...
import { getUserFilePath, s3, uploadFromStream } from '.';
...
export async function saveAsset(
  ...
  const filePath = getUserFilePath(author, filename);
...
export async function deleteAsset(name: string, author: string): Promise<ManagedAssetResponse | null> {
  ...
  await s3.deleteObject({ Bucket: process.env.ASSET_BUCKET_NAME, Key: getUserFilePath(author, asset.fileName) });
