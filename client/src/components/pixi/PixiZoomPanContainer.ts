...
export class PixiZoomPanContainer extends Container {
  ...
  private onZoom(ev: WheelEvent) {
    ...
    const newScale = Math.round(Math.max(
      0.2,
      this.scale.x * (1 +this.zoomDirection(ev) * 0.15)) * 100) / 100;

    this.xZoomOffset = (this.xZoomOffset - x) *
      (newScale / this.scale.x) + x;
    this.yZoomOffset = (this.yZoomOffset - y) *
      (newScale / this.scale.x) + y;
    this.setTransform(
      this.xZoomOffset, this.yZoomOffset, newScale, newScale
    );
  }
